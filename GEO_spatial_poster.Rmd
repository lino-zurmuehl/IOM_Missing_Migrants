...
migration_connections <- tribble(
  ...
  "SRB", "ROU", "Western Balkan", # TODO: Change the 'volume' into one number (no longer need to be based on mm_df aggragted)
)
...
text(x = 1, y = positions, labels = aggregated_mm_df_ordered$count, col = "white", font = 2, cex = 3.4)
...
combined_migration_violence_map <- ggplot() +
  ...
  labs(
    ...
    subtitle = "Migration flows and average annual violent incidents per million inhabitants",
    caption = "Violance score is the sum of average annual violent incidents per million inhabitants\nalong the migration route countries. \nData source: ACLED, IOM, UNHCR, 2023",
  ) +
  # Set map bounds
  coord_sf(  xlim = c(-20, 85),  
             ylim = c(-15, 65),    
...
par(bty = "n", mar = c(2, 2, 4, 2))  # Adjust margins for better spacing
plot(
...
route_comparison_plot <- ggplot(route_violence_scores, aes(x = reorder(group, total_violence), y = total_violence)) +
  ...
  coord_flip() +
  labs(
    title = "Total Violence Score along the Migration Route Countries",
    x = "",
    y = "Sum of Average Annual Violent Incidents per Million Inhabitants",
  ) +
  theme_minimal() +
  theme(
...
ggsave("route_violence_comparison.png", route_comparison_plot, width = 12, height = 6, dpi = 300) # TODO: Add Number of deaths per route. 
```
