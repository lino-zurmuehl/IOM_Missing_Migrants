...
# Join the violent incidents data with the countries data
# We need to map country names to ISO3 codes
# Creating a mapping table
country_name_to_iso3 <- tribble(
  ~country, ~ISO3_CODE,
  "Syria", "SYR",
  "Turkey", "TUR",
  "Afghanistan", "AFG",
...
# Join the ISO3 codes to the violent incidents data
violent_incidents_with_iso3 <- acled_df_new %>%
  left_join(country_name_to_iso3, by = "country")
...
countries_with_types_and_incidents <- countries_with_types %>%
  left_join(violent_incidents_with_iso3, by = "ISO3_CODE") %>%
  mutate(
    avg_per_year = ifelse(is.na(avg_per_year), 0, avg_per_year)
  )
...
route_countries <- migration_connections %>%
  ...
  left_join(violent_incidents_with_iso3, by = c("all_countries" = "ISO3_CODE")) %>%
